...
当前阶段：Phase 16 — 题目投稿与 AI 题库生产 🚧 开发中（Round 83 完成 P0 投稿中心修复加固，后端 232 个测试）
...
- [ ] Phase 16：题目投稿与 AI 题库生产 🚧（P0 题目投稿中心 ✅：投稿表 Flyway V7 ✅、用户投稿提交 ✅、用户投稿列表 ✅、管理端投稿列表 ✅、管理员审核 ✅、投稿入库 ✅、投稿统计 ✅、前端页面 ✅、投稿链路修复加固 ✅；待做：AI 题目质检、AI 知识点标注、AI 难度评估、管理员 AI 审核辅助）
...
- Phase 13 全部完成（Round 68-72），Phase 14 P0 文本可视化已完成（Round 73），P1 Mermaid 流程图已完成（Round 74），Phase 15 P0 学习诊断已完成（Round 75），AI 个性化学习建议已完成（Round 76），相似题推荐已完成（Round 77），Phase 16 P0 题目投稿中心已完成（Round 82），投稿链路修复加固已完成（Round 83）
- 缓存 TTL 参数目前硬编码在 RedisConfig 中，后续可抽为 application.yml 配置项
...
任务名称：Phase 16 P1 — AI 题目质检与审核辅助

Phase 16 Round 82 已完成 P0 题目投稿中心（投稿提交、审核、入库全流程，含 Flyway V7、后端 9 个新文件、前端 3 个新文件 + 路由导航更新）。Round 83 已修复前端 API 返回契约、判断题投稿答案、单选题正确项校验，以及填空/简答入库后正式判分读取问题，并补充后端投稿服务测试。

下一步建议：
- 继续 Phase 16 P1 方向：AI 题目质检（AI 辅助检查投稿题目质量、格式规范、答案/解析完整性）
- 再做管理员审核辅助：AI 给出风险点、修改建议和推荐审核意见，但不自动发布
...
当前阶段：Phase 16 — 题目投稿与 AI 题库生产（P0 题目投稿中心已完成并完成修复加固）。

已完成模块：用户鉴权、课程知识点、题库、刷题判分（含填空简答增强）、错题本（含重练）、试卷考试、AI 功能（含流式输出与配额管理）、统计可视化（含个人学习报告）、质量提升、部署简历、收藏题练习、Excel 导入导出、学习计划、题目评论、多端适配、难度自适应、管理端用户管理、Phase 13 AI 题目学习资产、Phase 14 P0/P1 可视化讲解、Phase 15 学习画像与推荐、Phase 16 P0 题目投稿中心、后端测试 232 个、前端 Vitest 测试（187 个，21 个测试文件）、GitHub Actions CI。
下一阶段主线：Phase 16 P1，围绕 AI 题目质检与管理员审核辅助做小步迭代，先检查投稿题目的质量、格式、答案/解析完整性，再考虑知识点标注和难度评估。
