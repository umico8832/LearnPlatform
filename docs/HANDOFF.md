...
- [x] Phase 12：体验增强迭代（🔵 已完成 AI 题目助手、管理端统计、AI 流式输出、用户个人中心、错题重练、收藏题练习、Excel 导入导出、学习计划、AI 调用日志、核心业务可信度修复与部分后端核心服务测试）
...
14. **体验增强**：错题重练、题目收藏、收藏题练习、Excel 导入导出、每日学习计划
...
18. **AI 用户级限流**：每日调用配额（默认 50 次/天），所有同步/流式接口受保护，前端自动展示配额超限提示
19. **后端核心测试补充**：已覆盖 JWT、答案判分、考试关键校验、刷题提交与错题本核心规则，共 44 个后端测试通过
...
已完成模块：用户鉴权、课程知识点、题库、刷题判分、错题本、试卷考试、AI 功能、统计可视化、质量提升、部署简历、收藏题练习、部分后端核心服务测试。
后续扩展方向：见 docs/FUTURE.md；下一步建议完善“个人学习报告”，再继续 CI 和演示素材。
