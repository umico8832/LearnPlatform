...
下一阶段主线：继续完成演示截图与 CI 实跑，并补充少量高价值 E2E；随后依次实现真实 token/成本、用户独立配额、调用报告与异常提醒。内容复审缓存、重复题检测和学习效果指标属于后续衔接。OCR、爬虫、自动入库和复杂推荐仍非当前优先级。
...
- [x] Phase 19：AI 调用分析与成本控制 ✅ 基本完成（调用趋势、功能/模型分布、Top 用户、失败调用、Tokens 与平均耗时）
- [ ] Phase 20：演示验收与 AI 运营治理 🚧（已完成：考试完整浏览器验收、考试页交互回归、登录/课程浏览/刷题错题闭环浏览器 E2E；待做：截图、CI 实跑、考试与管理端关键 E2E、真实 token/成本、用户独立配额与调用报告）
...
- 已建立隔离的 `e2e` Profile，并以 Playwright 覆盖真实账号密码、验证码、JWT、课程浏览和“刷题答错→错题本→掌握度更新→重练”闭环；考试与管理端审核流程仍待覆盖。若普通 Docker 环境已启动，按 `docs/TESTING.md` 使用 `--force-recreate` 切换到 E2E Profile。
- `tokensUsed` 尚未从上游 API 提取，当前 AI 用量不能用于真实成本核算。
...
3. 复用现有安全 E2E 登录态，为考试和管理端审核补充少量高价值端到端测试。
4. 完成验收收尾后，优先做真实 token/成本、用户独立配额、调用报告与异常提醒。

Round 105 已验证 Docker Redis 网络连接、8 个缓存/管理接口及前端代理的 AI 用量接口均返回 200；Round 106 已完成核心浏览器点击验收并修复刷题结果弹窗闪屏；Round 108 已完成考试全流程浏览器验收；Round 110 已接入首条 Playwright E2E；Round 111 已补齐刷题错题闭环 E2E 并实际验证 2 条浏览器用例。
...
建议 commit message: `test(e2e): 覆盖刷题错题重练闭环`
...
当前阶段：Phase 19 已基本完成，Phase 20“演示验收与 AI 运营治理”正在进行。Round 105 已修复 Docker Redis 缓存与 AI 用量接口路径，Round 106 已完成核心浏览器点击验收并修复刷题结果弹窗的关闭动画闪屏，Round 108 已完成考试完整作答、提交和结果查看的真实 Docker 浏览器验收，Round 109 已补考试页交互回归测试，Round 110-111 已补 Playwright 登录/课程与刷题错题闭环 E2E。
...
3. 复用现有安全 E2E 登录态，补充考试和管理端审核流程；并处理 Testcontainers 兼容性与缓存 TTL 配置化。
4. 若继续迭代，按“真实 token/成本 → 用户独立配额 → 调用报告与异常提醒 → 内容质量与学习效果指标”的顺序推进。

当前验收基线：后端 `mvn test` 341 passed；前端 `npm test` 192 passed、`npm run build` 成功；核心用户与管理员页面已通过真实浏览器点击验收。
