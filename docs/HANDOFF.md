...
- 运营提醒已在报告生成时持久化为 `ai_usage_alert`，同类型、同周期天数、同一天生成的未确认提醒会复用同一条记录；管理员可在 AI 调用分析页或顶部栏提醒下拉中确认提醒。当前尚未发送邮件、Webhook 等站外通知。
...
3. 继续后续缓存/运营治理细节，例如站外通知、重复题检测或学习效果指标。
