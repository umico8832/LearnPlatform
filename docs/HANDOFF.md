...
当前阶段：Phase 19 — AI 调用分析与成本控制面板 ✅ 基本完成；Phase 20 — 演示验收与 AI 运营治理 🚧 开发中（用户独立配额已完成）。

下一阶段主线：继续完成演示截图与 CI 实跑；AI 调用日志已接入真实 token 和按配置模型单价计算的成本，且管理员可设置用户独立配额，随后实现调用报告与异常提醒。内容复审缓存、重复题检测和学习效果指标属于后续衔接。OCR、爬虫、自动入库和复杂推荐仍非当前优先级。
...
- [ ] Phase 20：演示验收与 AI 运营治理 🚧（已完成：考试完整浏览器验收、考试页交互回归、登录/课程浏览/刷题错题闭环/考试作答/投稿审核入库浏览器 E2E、真实 token 与模型成本日志、管理员用户独立配额；待做：截图、CI 实跑、调用报告与异常提醒）
...
3. 完成验收收尾后，优先做调用报告与异常提醒，并为配额调整补充审计记录。
...
已完成模块：用户鉴权、课程知识点、题库、刷题判分（含填空简答增强）、错题本（含重练）、试卷考试、AI 功能（含流式输出、真实 token/成本日志和按用户独立的日配额）、统计可视化（含个人学习报告）、质量提升、部署简历、收藏题练习、Excel/Markdown 导入导出、学习计划、题目评论、多端适配、难度自适应、管理端用户管理、Phase 13 AI 题目学习资产、Phase 14 全部可视化讲解（13 种元素类型）、Phase 15 的学习诊断/建议/规则相似题与错因分析、Phase 16 的投稿/AI 质检/标注/难度评估/来源追踪与复审机制、Phase 17 间隔重复复习系统（SM-2 算法）、Phase 18 全局搜索与快捷导航（核心功能）、后端测试 354 个、前端 Vitest 测试（最新已验证 194 个）、GitHub Actions CI 配置。
下一步建议：
1. 制作用户端与管理端演示截图。
