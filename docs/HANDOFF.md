...
当前阶段：Phase 16 — 题目投稿与 AI 题库生产 🚧 开发中（Round 85 完成 P1 AI 知识点标注，后端 247 个测试）
...
- [ ] Phase 16：题目投稿与 AI 题库生产 🚧（P0 题目投稿中心 ✅：投稿表 Flyway V7 ✅、用户投稿提交 ✅、用户投稿列表 ✅、管理端投稿列表 ✅、管理员审核 ✅、投稿入库 ✅、投稿统计 ✅、前端页面 ✅、投稿链路修复加固 ✅；P1 AI 题目质检与审核辅助 ✅：5 维度 AI 质检 ✅、降级回退基础规则检查 ✅、管理端质检对话框 ✅、后端单元测试 7 个 ✅；P1 AI 知识点标注 ✅：AI 推荐知识点（置信度+理由）✅、一键应用到投稿 ✅、降级关键词匹配 ✅、管理端标注对话框 ✅、后端单元测试 8 个 ✅；待做：AI 难度评估、质检结果缓存、标注结果缓存）
...
- Phase 13 全部完成（Round 68-72），Phase 14 P0 文本可视化已完成（Round 73），P1 Mermaid 流程图已完成（Round 74），Phase 15 P0 学习诊断已完成（Round 75），AI 个性化学习建议已完成（Round 76），相似题推荐已完成（Round 77），Phase 16 P0 题目投稿中心已完成（Round 82），投稿链路修复加固已完成（Round 83），P1 AI 题目质检与审核辅助已完成（Round 84），P1 AI 知识点标注已完成（Round 85）
- 缓存 TTL 参数目前硬编码在 RedisConfig 中，后续可抽为 application.yml 配置项
...
任务名称：Phase 16 P1 — AI 难度评估或质检/标注体验优化

Phase 16 Round 85 已完成 P1 AI 知识点标注（SubmissionKPTaggingService AI 推荐知识点 + 一键应用 + 降级关键词匹配 + 管理端标注对话框 + 8 个单元测试，后端 247 个测试全部通过）。

下一步建议：
- 继续 Phase 16 P1 方向：AI 难度评估（基于题目内容自动评估难度）
...
当前阶段：Phase 16 — 题目投稿与 AI 题库生产（P0 投稿中心 + P1 AI 质检与审核辅助已完成）。

已完成模块：用户鉴权、课程知识点、题库、刷题判分（含填空简答增强）、错题本（含重练）、试卷考试、AI 功能（含流式输出与配额管理）、统计可视化（含个人学习报告）、质量提升、部署简历、收藏题练习、Excel 导入导出、学习计划、题目评论、多端适配、难度自适应、管理端用户管理、Phase 13 AI 题目学习资产、Phase 14 P0/P1 可视化讲解、Phase 15 学习画像与推荐、Phase 16 P0 题目投稿中心、Phase 16 P1 AI 质检与审核辅助、后端测试 239 个、前端 Vitest 测试（187 个，21 个测试文件）、GitHub Actions CI。
下一阶段主线：Phase 16 P1 继续，围绕 AI 知识点标注和难度评估做小步迭代，或优化质检体验（缓存结果、一键填充审核意见）。
