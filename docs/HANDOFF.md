...
当前阶段：Phase 19 — AI 调用分析与成本控制面板 ✅ 基本完成；Phase 20 — 演示验收与 AI 运营治理 📋 已规划。

下一阶段主线：先完成考试端到端验收、演示截图与 CI 实跑；随后依次实现真实 token/成本、用户独立配额、调用报告与异常提醒。内容复审缓存、重复题检测和学习效果指标属于后续衔接。OCR、爬虫、自动入库和复杂推荐仍非当前优先级。
...
- [ ] Phase 20：演示验收与 AI 运营治理 📋（待做：考试完整浏览器验收、截图、CI 实跑、关键 E2E、真实 token/成本、用户独立配额与调用报告）
...
- 考试完整作答、提交与结果查看尚未完成浏览器验收；项目真实演示截图尚未制作。
- GitHub Actions 已配置，但仍需推送后确认后端、前端和 Docker Job 的实际结果。
- 前端页面级组件仍是部分覆盖，尚未建立关键业务浏览器级 E2E 测试。
...
1. 补做考试完整作答流程的浏览器验收，并按 `docs/DEMO.md` 制作真实截图。
2. 推送至 GitHub 后确认 Actions CI 的实际运行结果。
3. 为登录、刷题、错题复习、考试和管理端审核补充少量高价值端到端测试。
...
建议 commit message: `docs: 校准后续路线为演示验收与 AI 运营治理`
...
当前阶段：Phase 19 已基本完成，Phase 20“演示验收与 AI 运营治理”已规划。Round 105 已修复 Docker Redis 缓存与 AI 用量接口路径，Round 106 已完成核心浏览器点击验收并修复刷题结果弹窗的关闭动画闪屏。

已完成模块：用户鉴权、课程知识点、题库、刷题判分（含填空简答增强）、错题本（含重练）、试卷考试、AI 功能（含流式输出与全局日配额）、统计可视化（含个人学习报告）、质量提升、部署简历、收藏题练习、Excel/Markdown 导入导出、学习计划、题目评论、多端适配、难度自适应、管理端用户管理、Phase 13 AI 题目学习资产、Phase 14 全部可视化讲解（13 种元素类型）、Phase 15 的学习诊断/建议/规则相似题与错因分析、Phase 16 的投稿/AI 质检/标注/难度评估/来源追踪与复审机制、Phase 17 间隔重复复习系统（SM-2 算法）、Phase 18 全局搜索与快捷导航（核心功能）、后端测试 341 个、前端 Vitest 测试（最新已验证 190 个）、GitHub Actions CI 配置。
下一步建议：
1. 补做考试完整作答流程的浏览器验收，并制作演示截图。
2. 推送至 GitHub 后确认 Actions CI 实际运行结果。
3. 补充关键业务端到端测试，并处理 Testcontainers 兼容性与缓存 TTL 配置化。
