...
当前阶段：Phase 16 — 题目投稿与 AI 题库生产 🚧 开发中（Round 84 完成 P1 AI 质检与审核辅助，后端 239 个测试）
...
- [ ] Phase 16：题目投稿与 AI 题库生产 🚧（P0 题目投稿中心 ✅：投稿表 Flyway V7 ✅、用户投稿提交 ✅、用户投稿列表 ✅、管理端投稿列表 ✅、管理员审核 ✅、投稿入库 ✅、投稿统计 ✅、前端页面 ✅、投稿链路修复加固 ✅；P1 AI 题目质检与审核辅助 ✅：5 维度 AI 质检 ✅、降级回退基础规则检查 ✅、管理端质检对话框 ✅、后端单元测试 7 个 ✅；待做：AI 知识点标注、AI 难度评估、质检结果缓存）
...
- Phase 13 全部完成（Round 68-72），Phase 14 P0 文本可视化已完成（Round 73），P1 Mermaid 流程图已完成（Round 74），Phase 15 P0 学习诊断已完成（Round 75），AI 个性化学习建议已完成（Round 76），相似题推荐已完成（Round 77），Phase 16 P0 题目投稿中心已完成（Round 82），投稿链路修复加固已完成（Round 83），P1 AI 题目质检与审核辅助已完成（Round 84）
- 缓存 TTL 参数目前硬编码在 RedisConfig 中，后续可抽为 application.yml 配置项
...
任务名称：Phase 16 P1 — AI 知识点标注或难度评估

Phase 16 Round 84 已完成 P1 AI 题目质检与审核辅助（SubmissionAiQualityService 5 维度 AI 质检 + 降级回退 + 管理端质检对话框 + 7 个单元测试，后端 239 个测试全部通过）。

下一步建议：
- 继续 Phase 16 P1 方向：AI 知识点标注（自动为投稿推荐知识点）或 AI 难度评估（基于题目内容自动评估难度）
