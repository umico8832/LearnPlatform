...
下一阶段主线：继续完成演示截图与 CI 实跑；真实 token 已接入调用日志，随后实现模型成本、用户独立配额、调用报告与异常提醒。内容复审缓存、重复题检测和学习效果指标属于后续衔接。OCR、爬虫、自动入库和复杂推荐仍非当前优先级。
...
- [ ] Phase 20：演示验收与 AI 运营治理 🚧（已完成：考试完整浏览器验收、考试页交互回归、登录/课程浏览/刷题错题闭环/考试作答/投稿审核入库浏览器 E2E、上游真实 token 日志；待做：截图、CI 实跑、模型成本、用户独立配额与调用报告）
...
- 已从 OpenAI 兼容上游响应记录真实 `tokensUsed`；流式用量默认通过 `stream_options.include_usage` 请求，无法支持该扩展的上游可设 `AI_STREAM_INCLUDE_USAGE=false`，对应日志保持空值而不估算。模型单价和精确成本仍待实现。
...
3. 完成验收收尾后，优先做模型成本、用户独立配额、调用报告与异常提醒。

Round 105 已验证 Docker Redis 网络连接、8 个缓存/管理接口及前端代理的 AI 用量接口均返回 200；Round 106 已完成核心浏览器点击验收并修复刷题结果弹窗闪屏；Round 108 已完成考试全流程浏览器验收；Round 110 已接入首条 Playwright E2E；Round 111 已补齐刷题错题闭环 E2E；Round 112 已补齐考试作答、提交及结果详情 E2E；Round 113 已补齐投稿审核入库 E2E；Round 114 已接入上游真实 token 用量记录。
...
建议 commit message: `feat(ai): 记录上游真实 token 用量`
...
当前阶段：Phase 19 已基本完成，Phase 20“演示验收与 AI 运营治理”正在进行。Round 105 已修复 Docker Redis 缓存与 AI 用量接口路径，Round 106 已完成核心浏览器点击验收并修复刷题结果弹窗的关闭动画闪屏，Round 108 已完成考试完整作答、提交和结果查看的真实 Docker 浏览器验收，Round 109 已补考试页交互回归测试，Round 110-113 已补 Playwright 登录/课程、刷题错题、考试判分及投稿审核入库闭环 E2E，Round 114 已将 OpenAI 兼容上游的真实 token 用量写入统一 AI 调用日志。
...
4. 若继续迭代，按“模型成本 → 用户独立配额 → 调用报告与异常提醒 → 内容质量与学习效果指标”的顺序推进。

当前验收基线：后端 `mvn test` 347 passed；前端 `npm test` 192 passed、`npm run build` 成功；核心用户与管理员页面已通过真实浏览器点击验收。
