...
- 选择页面后点击“开始预览”才加载，可切换 1280、1440、1920 像素桌面宽度、刷新、停止或在新标签打开。
- 课程、考试记录与试卷学习页面需填写当前账号有权访问的真实 ID；也可通过“从业务入口进入”建立所需上下文。
  练习答题页需要先选题，AI 教学页进入时可能创建会话，考试页会计时；预览中的业务操作会实际生效。
- 管理端默认连接 `http://localhost:5174/admin/`，需单独启动管理端并登录；使用其他地址时可在工具中修改。
- 登录权限保持原样；预览不是权限绕过或离线模拟。认证页面的多步骤、加载与结果状态继续使用
  [认证状态预览](http://localhost:5173/dev/auth-preview)，两个工具可互相跳转。
