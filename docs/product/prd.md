...
系统根据未完成教学、到期复习、重要错误与课程默认顺序形成候选目标，AI 在该范围内选择讲法、例子、
理解检查与已注册课件；真实作答或操作形成结构化事件，再决定继续、补前置、换讲法或安排复习。
不把完整历史交给模型后依赖其自行记忆进度，也不把用户自评或“懂了”直接视为掌握事实。

互动课件用于确有教学价值的状态变化与算法过程；只选择已注册课件及校验后的场景数据，
不执行模型生成的任意代码。高成本教学与生成应有配额、取消、重试和降级边界。
上游不可用时，已有原题、考试、错题与复习仍可使用。生成、审查、Prompt 与模型配置保留版本或 trace，
不保存原始秘密；没有实验设计时，学习效果仅表达观察性关联。
...
以下审查 Agent 描述是产品目标，不表示当前已实现自动审查发布。现有正式题发布仍以
[管理员审查 API](../reference/api/admin-governance.md#ai-变式题审查)为准；不得据此绕过管理员权限。
...
- 登录用户可以围绕考试科目、知识主题发起共享讨论、评论回复与点赞，并按分类、学校或关键词检索。
- 用户可以贡献知识解释、类比、易错点、做题方法、大纲和题库附件，投稿保留来源与使用许可说明。
- 学校分区由管理员创建；资料未公开前仅作者与管理员可见，管理员可以审核、驳回或隐藏内容。
- 从社区资料整理的题目仍进入既有单题投稿审核与显式入库流程，不自动进入正式题库或权威知识库。
