...
| `user_identity` | 用户与第三方认证主体的稳定关联 | 提供方主体唯一；同一用户每个提供方最多一个身份 |
| `oauth_login_ticket` | 第三方认证回调与 JWT 签发之间的一次性票据 | 只保存票据 HMAC；两分钟内仅可消费一次 |
...
| `tutor_agent_plan_confirmation` | 用户显式采用 Agent 已保存安排 | 消息唯一；确认不表示学习完成，事务见[AI 数据](ai-and-governance.md) |
...
| 表 | 职责 | 关键约束 |
|---|---|---|
| `community_category` | 考试、科目和学校分区目录 | 分类标识唯一，同类名称唯一，科目引用父考试 |
| `community_post` | 共享讨论和学习资料、来源与当前审核状态 | 作者来自登录账号；课程与知识点关系由服务校验 |
| `community_attachment` | 原文件字节、文件名与大小 | 创建时和内容原子提交；按作者行锁校验存储配额 |
| `community_comment` | 评论及同话题回复关系 | 删除清空正文并保留占位；拒绝跨话题回复 |
| `community_like` | 内容与评论的真实点赞关系 | `(post_id, comment_id, user_id)` 唯一；`comment_id=0` 表示内容点赞 |
| `community_review` | 内容审核审计 | 追加记录，不覆盖过去的审核人、结论与说明 |
| `community_question_link` | 社区资料与既有单题投稿的关联 | 单题投稿唯一关联来源；`(post_id, request_key)` 保证整理请求幂等 |

文件与正文存于同一 MySQL 事务，避免附件元数据指向未保存的原文件；当前不依赖对象存储或相邻 AiStu 目录。
社区表属于贡献内容域，不进入权威知识快照或用户学习事实。审核、回复、点赞、删除与整理题目在内容行锁下串行化。
作者删除内容会释放原附件字节，保留逻辑删除的正文来源记录与已整理题目之间的关系。
公开状态、格式及配额的调用方契约统一见[社区 API](../api/community.md)。
