...
V101 新增 `tutor_agent_practice_attempt`，以 `message_id` 唯一绑定一次推荐的首次作答，
`practice_record_id` 同样唯一；`question_json` 固化首次作答题面和公开选项，`result_json` 固化判分及解析，
后续题目修改不替换已答题面或重判已有结果；恢复仍先核对当前题目的访问资格。
提交使用 READ COMMITTED 短事务，锁定推荐的 ASSISTANT 消息后复核题目资格及既有结果；
