...
| `learning_plan` | V1，V3 补齐 | 历史学习目标数据；当前无运行时入口 | V3 使用幂等创建兼容旧数据库；表保留以避免破坏既有数据 |
...
| `exam_learning_answer` | V76，V92 扩展 | 学习会话中每题的每次尝试、判分和耗时 | 会话、题目和尝试序号组合唯一；主观题未判分时正确性和得分为空 |
| `exam_learning_ai_interaction` | V79 | 基于试卷学习最近一次作答的 AI 辅导交互 | 绑定用户、课程、试卷、会话、题目与答案；只保存状态和引用 |
| `user_exam_source` | V83，V85 扩展 | 用户导入草稿或已确认试卷的资料名称、格式、原文件/内容哈希、解析全文及受限原文件 | 绑定所有者；正文和文件只由所有者接口读取；普通查询不加载二进制列 |
...
`exam_learning_session` 与考试记录采用独立状态机，通过用户、试卷与 `active_session_key` 唯一约束
保证并发开始只形成一个进行中会话。逐题提交事务锁定会话、校验归属和状态、递增尝试序号并追加
`exam_learning_answer`，可判分结果与错题、复习计划及课程事件在同一事务内保存。
V92 允许主观题自评的 `is_correct` 与 `score` 为 `NULL`，使数据库与 `SELF_REVIEW` 响应语义一致；
历史正确性和分数不根据题型推测或改写。
它不创建 `exam_answer` 或考试成绩。

完成事务固化状态与完成时间，将活动键置空；会话及全部尝试继续保留。完成条件、答案开放与主观题
自评语义见[试卷学习 API](../api/exams.md#用户端试卷学习)。

`exam_learning_ai_interaction` 绑定用户、会话、题目及最近一次答案，保存业务引用、失败摘要和状态：
`0` 处理中、`1` 成功、`2` 失败。`answer_correct` 保存调用时的判分快照：正确为 `1`、错误为 `0`、
主观题自评尚未判分为 `NULL`；V92 不推测或改写历史 `0/1` 快照。成功交互以自身 ID 形成幂等课程事件，
不存储完整提示词或模型输出。
AI 调用前提与辅导类型由上述 API 契约维护。
...
DOCX 同样以原文件字节计算哈希，正文只保存 Apache POI 提取的普通段落和表格文本。V85 起，确认或创建
草稿时还会把最大 10MB 的 PDF/DOCX 原字节、规范媒体类型和长度保存在受限二进制列；普通资料查询不加载
该列，下载时重新核对所有者、长度、媒体类型和 SHA-256。Markdown/文本没有伪造的原文件下载。
预览、确认和草稿创建继续使用同一哈希连续性约束。

原文件累计容量不另建可漂移的计数表，而是按所有者汇总 `source_size`。文件写入事务先对 `user` 所有者行
执行 `SELECT ... FOR UPDATE`，再计算已用字节并应用可配置配额；同一所有者的并发写入因此串行校验，来源
记录删除后无需补偿任务即可释放容量。用量和文件数查询只读取元数据列。
所有者存储清单同样不读取 `original_content` 或 `source_file`，按来源创建时间倒序分页，并以所有者条件
关联 `private_exam_import_draft` 与有效私有 `exam_paper`；已确认来源优先展示正式试卷，未确认来源展示
草稿，跨账号来源不会进入结果。清单本身不提供绕过业务对象引用的直接删除 SQL。
...
`user_exam_source`（包括 V85 原文件二进制）只有在不存在任何草稿或有效试卷引用时才随之删除。已有学习事实的私有试卷拒绝删除，
