...
- 按产品、架构、开发、参考、项目状态和展示材料分类的文档体系，以及项目 Skill 与第三方 Skill 的明确边界。
...
- 3 个项目 Skill 位于 `.agents/skills`，均包含 `agents/openai.yaml` 并通过系统校验。
- 第三方 `.codex/skills` 保持上游安装内容不变。
...
- 第三方 `.codex/skills` 缺少可验证的上游 URL、安装命令和版本锁，且部分工作流声明了当前未安装的辅助 Skill；更新或使用相关能力时必须先核对真实依赖。
