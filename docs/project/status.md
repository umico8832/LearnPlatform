...
- `QuestionLearningAsset.vue` 从 687 行降为 521 行、响应式状态从 15 处降为 12 处；单类资产反馈读取、
  评价和补充说明进入 `QuestionAssetFeedback`。
- 新增 2 个反馈组件测试，连同资产可见性与训练共 4 个测试通过；相关 ESLint 与 TypeScript 检查通过，
  反馈失败不阻断学习内容的语义不变。

逐卡复习会话状态机拆分（Round 307，2026-09-05）：

- `ReviewView.vue` 从 681 行降为 456 行、响应式状态从 18 处降为 9 处；逐卡答题与完成状态进入
  `ReviewSessionPanel`，页面保留查询、计划维护、统计与 AI 建议协调。
- 新增会话组件测试，连同路由目标复习共 3 个测试通过；相关 ESLint 与 TypeScript 检查通过，
  到期筛选、提交判定和刷新语义不变。

试卷编辑与选题业务闭包拆分（Round 306，2026-09-05）：

- `ExamManage.vue` 从 692 行降为 204 行；新增 / 编辑、详情映射、选题与保存进入
  `ExamPaperEditorDialog`，纯模型统一官方来源发布约束和请求映射。
- 新增 2 个模型测试；相关 ESLint、TypeScript 与双生产构建通过，API、提示、保存和刷新语义不变。
...
- `QuestionManage.vue`、`QuestionListView.vue` 分别从 825、819 行降为 697、644 行；分页筛选、选择、
  收藏、评论展开、纠错和列表级命令进入两个独立 composable。
- 新增 2 个 composable 测试文件、4 个测试，相关 ESLint 与 TypeScript 检查通过；当前全部 Vue SFC
  少于 700 行，后端 Service 最高 388 行。
...
- `AiExamGenerationService` 从 590 行、7 个依赖降为 150 行、4 个依赖；候选事实加载、均衡选题、
  预览展示和试卷创建分别进入 `service/exam` 协作者。
- 新增 10 个专属测试，相关管理端 Controller、ArchUnit 与后端 57 个聚焦回归测试通过；公共 API、
  候选范围、权重和事务创建行为不变。

课程总览展示职责拆分（Round 285，2026-09-04）：

- `CourseOverviewView.vue` 从 937 行降为 260 行，只保留路由、API 和阶段测评编排；新增 592 行的
  `CourseOverviewContent.vue` 承载加载 / 失败 / 空数据、目录、工具和学习事实展示。
- 总览 16 个聚焦测试、前端 ESLint 与学习 / 管理双生产构建通过；接口、事件、Design Tokens 和
  移动端断点不变。

通用 AI 门面职责拆分（Round 284，2026-09-04）：

- `AiService` 从 502 行、8 个依赖、19 个公开方法降为 102 行、3 个依赖的兼容门面；题目辅导、
  复习建议、知识总结和带配额审计的 Provider 调用分别独立。
- 相关 13 个聚焦测试通过；`mvn clean verify -B` 后端完整门禁通过，API、Prompt、题目可见性、
  配额与调用审计语义不变。

考试与私有试卷核心编排职责拆分（Round 283，2026-09-04）：

- `ExamService` 从 473 行、11 个依赖降为 66 行、3 个依赖，考试会话、提交、判分事实与结果展示分离；
  `CourseStageAssessmentService` 从 365 行、10 个依赖降为 207 行、7 个依赖，快照、事实回写与视图分离。
- `PrivateExamImportService` 从 454 行、9 个依赖降为 151 行、6 个依赖，解析校验与确认持久化分离；
  API、数据库、事务、答案脱敏、所有者隔离、来源哈希与文件配额行为不变。
- 相关聚焦测试通过；`mvn clean verify -B` 后端完整门禁通过。
...
- 新增 94 行的 `ExamPaperLearningContextService`，集中校验试卷可见性、发布状态、课程绑定、
  个人课程库关系与题目课程归属，并按既有顺序读取试卷题目和选项；`ExamPaperLearningService`
  从 392 行收敛为 320 行，直接依赖从 12 个降为 8 个。
- 主服务继续拥有会话创建 / 恢复、逐题判分、完成状态、错题与复习联动、课程学习事件和缓存清理事务；
  上下文、主服务、AI 辅导与控制器聚焦 4 个测试类 24 个测试通过。
- 后端全量 583 个测试通过；ArchUnit、生产 / 测试双 Checkstyle、SpotBugs 和 JaCoCo 门禁通过。
  URL、DTO、数据库、事务、查询顺序、访问控制、答案脱敏和判分语义不变。
