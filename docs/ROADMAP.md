...
| 20 | 演示验收与 AI 运营治理 | 🚧 开发中 | 2-3 个迭代 | 已覆盖登录、课程、刷题错题闭环、考试及投稿审核入库的真实浏览器 E2E；已完成用户独立配额，继续完成演示截图、CI 实跑和调用报告 |
...
- [x] 支持管理员按用户调整 AI 配额：用户可继承全局 `ai.daily-quota`、设置 0 为不限次数，或设置 1-10000 次的覆盖值（Round 116）。调整原因和审计时间仍待后续引入。
