...
| 20 | 演示验收与 AI 运营治理 | 🚧 开发中 | 2-3 个迭代 | 已接入首条真实浏览器 E2E；继续完成演示截图、CI 实跑与关键流程覆盖，再补齐 AI 配额、真实成本和调用报告 |
...
- [ ] 为刷题、错题复习、考试和管理端审核补充少量高价值端到端测试（已补考试页 2 条页面级交互回归；浏览器 E2E 已覆盖登录、课程浏览及刷题答错→错题本→掌握度更新→重练闭环；仍待考试与管理端审核）。
- [ ] 处理 Testcontainers 本地兼容性，并将 Redis 缓存 TTL 提取为配置项。

### P1：AI 运营治理
- [ ] 从上游响应记录真实 token 用量；当前 `tokensUsed` 仅具备字段和聚合展示能力。
