...
| 22 | AI 学习效果验证 | 🚧 开发中 | 按真实数据迭代 | Round 158 完成资产查看与同题对照；Round 159 完成变式训练显式完成事件和管理端完成率；后续依据样本评估跨题/知识点效果 |

**Phase 0-12 预计总工期**：约 17-20 天

**当前阶段：Phase 20 与 Phase 21 已完成，Phase 22“AI 学习效果验证”持续推进。Round 159 已为每个用户与变式题缓存版本建立真实训练状态：内容可见记为开始，只有用户显式确认才记为完成；管理端按周期开始队列展示完成率，明确不代表自动判分。下一步先积累真实样本，再评估跨题知识迁移或结构化变式作答。向量推荐、OCR、爬虫和自动入库不在近期主线。**
...
- [ ] 若后续将变式题升级为结构化可判分题目，再增加变式训练正确率；当前不从 Markdown 阅读或完成按钮推断正确性。
- [ ] 基于资产类型和题目难度识别高价值/低价值内容，数据不足时不进入推荐策略。
