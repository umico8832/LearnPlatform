...
| 22 | AI 学习效果验证 | 🚧 开发中 | 按真实数据迭代 | Round 158-163 完成真实查看、变式判分、同题/跨题/资产类型观察与难度样本；Round 164 增加去重学习者覆盖门槛 |

**Phase 0-12 预计总工期**：约 17-20 天

**当前阶段：Phase 20 与 Phase 21 已完成，Phase 22“AI 学习效果验证”持续推进。Round 165 完成工程质量门禁、真实 MySQL CI 和首轮大模块拆分；Round 166 建立 AI Agent 任务 Prompt、风险驱动 TDD 协议、PR 证据清单和前后端覆盖率防退化门禁。学习效果主线继续积累真实样本，再评估课程或用户基础分层。向量推荐、OCR、爬虫和自动入库不在近期主线。**
...
- [x] 同题、跨题、单资产类型和变式难度样本均统计去重学习者数；比较组需同时达到 5 条作答与 3 位学习者，难度档也使用相同双门槛（Round 164）。
