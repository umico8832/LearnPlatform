...
| 22 | AI 学习效果验证 | 🚧 开发中 | 按真实数据迭代 | Round 158 已完成资产真实查看埋点、阅读后同题作答对照和管理端观察面板；后续依据样本再扩展跨题/知识点效果指标 |

**Phase 0-12 预计总工期**：约 17-20 天

**当前阶段：Phase 20 与 Phase 21 已完成，Phase 22“AI 学习效果验证”已启动。Round 158 新增真实资产查看记录，以同一用户、同一道题的首次查看时间切分后续作答和基线作答，并在管理端展示查看覆盖、有帮助率、两组正确率、样本量及观察性结论；任一组少于 5 条时不判断效果。向量推荐、OCR、爬虫和自动入库不在近期主线。**
...
- [ ] 增加变式训练完成率与后续正确率指标；先定义真实完成事件，不从 AI 调用日志推断。
