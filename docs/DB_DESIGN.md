...
Flyway V19 创建。公开的 `question_ai_asset.content` 只保存脱敏提示，真正的题干、选项、标准答案和提交后解析保存在本表；学习资产查询只返回不含答案与解析的安全 VO。首版固定生成单选题，但保留 `question_type` 便于后续扩展。管理端学习效果统计通过 `asset_id` 与 `ai_variant_training` 批量关联，只按真实首次判分汇总 `difficulty` 1-5 的样本分布，不把未作答生成记录计入难度正确率。
