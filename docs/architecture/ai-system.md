...
分级提示由 `request_tutor_hint` 检查当前作答状态，运行时根据完整持久历史分配 1–3 级，独立于送入模型的
最近 12 条消息。提示正文依据公开课节生成，等级动作随成功回复保存；重复工具调用、失败重试和刷新均不
额外推进等级。达到上限或已作答时不产生新提示动作，完成写回前复核作答状态，避免迟到提示进入历史。

变式训练通过只读 `recommend_tutor_practice` 选择已批准且属于本节的正式题，服务端保存 PRACTICE 动作。
用户显式打开和作答后，独立领域服务复用正式练习判分；`read_tutor_practice_result` 只读取绑定运行的真实结果。
模型没有批准、代作答或写学习事实的工具。资格与首次提交契约见[Tutor API](../reference/api/learning-content.md#tutor-会话)，
并发与快照约束见[AI 数据](../reference/database/ai-and-governance.md)。

计划编排通过只读 `propose_tutor_plan` 复用课程概览目标，服务端限定最多三步并保存 PLAN 动作。
用户通过独立确认接口表达采用意愿，模型不能确认或写学习事实；刷新恢复原安排，目标失效时禁用导航。
`read_tutor_plan_state` 绑定可信 run ID 核对最新计划的真实确认状态，不采信聊天自称。
确认时间、目标校验及并发约束分别见上述 API 与 AI 数据文档。

课程学习记忆由独立服务保存用户明确提交的目标和讲解偏好。Runtime 在每轮开始时重新校验课程权限并读取
最新快照，放在历史消息之后、当前问题之前的独立 USER 消息中；记忆内容不能进入 SYSTEM 规则。
模型没有修改记忆的工具，也不能把目标或自述投影为真实学习事实。纠正和删除只影响随后开始的提问，
不撤回运行中的快照或清理历史对话。存储、版本保护和删除语义见[课程学习记忆](../reference/api/learning-content.md#课程学习记忆)。

用户可为单个 Tutor 会话明确保存复盘，但它不是自动摘要。每轮开始时 Runtime 最多附入 5 条仍可用的复盘：
用户文字与服务端关联的该会话理解检查状态分字段传递，前者不能作为学习事实或指令，后者也只说明一次检查。
来源课节或知识点撤回后不再传递该复盘；复盘不汇入对话历史、变式或测评证据。版本、撤回和公开契约见
[Tutor 会话复盘](../reference/api/learning-content.md#tutor-会话复盘)。
...
Runtime 在读取记忆前、每次模型调用前、模型结果验收及每个工具执行前检查中断和执行有效性；
生产入口注入状态检查，按执行标识、消息序号、运行状态及数据库时间核对租约，不续租。
模型结果验收时失效或中断仍保留本次实际用量，分别记录失败或取消；工具阶段失效不改写此前已成功的模型审计。
