...
管理员访问 /api/admin/ai-usage/learning-effect
  ...
  → 排除原题重答和已有更早暴露的跨题对照样本
  → 合并 ai_asset_feedback 与周期内开始的变式训练队列
  → 通过 ai_variant_question.difficulty 将真实首次判分归入 1-5 难度档
  → 同题、跨题、资产类型和难度样本均同时统计作答量与去重学习者覆盖
  → 任一比较组需至少 5 条、3 位学习者；难度档也需 5 条、3 位学习者，且至少两个难度档达标后才提示可开始分层观察
  → 按资产类型首次查看时间再次切分同题作答，并返回每组作答数与去重学习者数
  → 类型样本允许因多资产暴露而重叠，仅作观察，不用于资产排名或自动推荐
  → 返回覆盖、反馈、训练完成率、结构化变式正确率与难度样本结构、同题/跨题/资产类型样本量和观察性结论
