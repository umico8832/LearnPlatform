...
LearnPlatform 当前是前后端分离的模块化单体 Web 应用。现有 Vue SPA 同时承载用户端
和管理端，Spring Boot 提供统一 REST/SSE API，MySQL 保存业务事实，Redis 提供缓存，
AI Provider 通过兼容 OpenAI 的接口接入。

已接受的迁移目标是将用户学习端和管理系统拆成两个独立 Vue 前端应用，并继续共用
同一 Spring Boot 后端、账号体系和业务数据。该目标不表示当前代码已经完成拆分。

## 系统上下文

```mermaid
flowchart LR
    User["学习者"] --> Learner["用户学习端（目标独立应用）"]
    Admin["管理员"] --> AdminWeb["管理系统（目标独立应用）"]
    Learner --> Nginx["Nginx"]
    AdminWeb --> Nginx
...
当前用户端和管理端仍在同一个前端工程中，通过路由和后端角色权限区分。迁移后两端
拥有独立入口、路由、布局和构建产物，但前端分离不构成权限边界；管理权限始终由
后端校验。后端继续保持模块化单体，不引入第二套后端、数据库、微服务、消息队列或
分布式事务。
...
6. 用户学习端与管理系统目标独立，后端权限、账号和数据保持统一。
