...
结构校验后的 AI 变式题先停留在独立审查队列。管理员核对母题课程边界、题干、选项、答案和解析后，
批准项才在同一事务内物化为 `AI_GENERATED` 正式题，并保留变式 ID 与母题 ID；驳回项不发布。阶段测评
只从正式已发布题目选题，并把来源链固化进会话快照；组卷本身不触发模型调用。
创建时还将题源归一为官方原题、平台人工题、用户私有题或 AI 生成题。详情、最近摘要和历史列表均从
快照聚合构成，不重新连接当前题目或试卷归属，因此内容治理变化不会重写既有测评的来源事实。
