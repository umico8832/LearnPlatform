...
- **数据统计**：学习趋势、正确率、知识点掌握雷达图、月度学习效果指标，以及 AI 资产真实查看、变式训练显式完成率、后续同题作答与知识点跨题迁移对照
...
Phase 20 演示验收与 AI 运营治理、Phase 21 前端信息架构与视觉体验优化均已完成，Phase 22 AI 学习效果验证持续推进。平台会记录用户实际看到的 AI 学习资产；变式题进入可见状态记为开始，只有用户显式确认才记为完成。管理端除同题作答对照外，还会排除原题重答，在相关阅读前后 30 天内观察共享知识点的跨题迁移。所有统计均展示样本量并明确为观察性数据，不把调用量、生成次数或完成按钮冒充自动判分。登录、课程、刷题错题、考试判分、投稿审核和高频页面巡检已有 5 条真实 Docker E2E 覆盖。后端默认测试 401 个、前端 Vitest 219 个均已通过；5 个 Testcontainers 集成测试类的 54 个真实 MySQL 用例全部通过。详见 [docs/ROADMAP.md](docs/ROADMAP.md)。
