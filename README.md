...
当前工作重点是完成真实演示截图与 CI 实跑验证；登录、课程、刷题错题闭环、考试作答判分及投稿审核入库均已有浏览器 E2E 覆盖。AI 调用日志已保存真实输入/输出 Token，管理员可在 `ai.model-prices` 配置模型的 USD/百万 Token 单价后查看成本聚合；未配置价格不会产生虚假成本。后续 AI 迭代将优先建设用户独立配额和调用报告等运营治理能力，详见 [docs/ROADMAP.md](docs/ROADMAP.md)。
