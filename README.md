...
当前工作重点是完成推送后的 CI 实跑验证，并推进 Phase 21 前端信息架构与视觉体验优化细节 polish；AppLayout 分组导航、全局样式变量、首页学习工作台、课程入口与详情页、核心学习页面、题库浏览页、管理端主要页面样板、用户/题目/投稿管理长操作列收纳、批量操作、空状态和真实演示截图已完成。登录、课程、刷题错题闭环、考试作答判分及投稿审核入库均已有浏览器 E2E 覆盖，且已在 Docker E2E 环境中复跑通过；后端默认测试 361 个通过，Testcontainers 真实 MySQL 集成测试 53 个通过。AI 调用日志已保存真实输入/输出 Token、请求追踪 ID、Prompt 模板名/不可逆指纹和模型配置版本指纹，不保存原始提示词；管理员可在 `ai.model-prices` 配置模型的 USD/百万 Token 单价后查看成本聚合，并可按用户设置每日调用配额（继承全局值或不限次数）；每次调整都必须填写原因，并保存管理员、前后值和时间的审计记录。未配置价格不会产生虚假成本。管理端 AI 调用分析支持 7/14/30/90 天运营报告，展示与前一等长周期的环比，并对高失败率、延迟和调用量异常给出实时提醒。详见 [docs/ROADMAP.md](docs/ROADMAP.md)。
