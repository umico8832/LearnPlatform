...
- **数据统计**：学习趋势、正确率、知识点掌握雷达图、月度学习效果指标等可视化面板
...
当前工作重点是完成推送后的 CI 实跑验证，并推进 Phase 21 前端信息架构与视觉体验优化细节 polish；AppLayout 分组导航、全局样式变量、首页学习工作台、课程入口与详情页、核心学习页面、题库浏览页、刷题记录复盘页、收藏题重点题库页、AI 复习建议页、个人中心账户与学习档案页、学习路径路线工作台、知识图谱学习工作台、学习报告效果面板、管理端主要页面样板、用户/题目/投稿管理长操作列收纳、批量操作、空状态、顶部 AI 运营提醒入口和真实演示截图已完成。登录、课程、刷题错题闭环、考试作答判分及投稿审核入库均已有浏览器 E2E 覆盖，且已在 Docker E2E 环境中复跑通过；个人学习报告已基于正确率变化、错题转化、复习掌握和活跃学习天数展示学习效果综合分、建议摘要和拆解进度指标；后端默认测试 387 个通过，Testcontainers 真实 MySQL 集成测试 53 个通过，前端 Vitest 214 个通过。AI 调用日志已保存真实输入/输出 Token、请求追踪 ID、Prompt 模板名/不可逆指纹和模型配置版本指纹，不保存原始提示词；管理员可在 `ai.model-prices` 配置模型的 USD/百万 Token 单价后查看成本聚合，并可按用户设置每日调用配额（继承全局值或不限次数）；每次调整都必须填写原因，并保存管理员、前后值和时间的审计记录。管理端 AI 调用分析支持 7/14/30/90 天运营报告，展示与前一等长周期的环比，并对高失败率、延迟和调用量异常生成可持久化、可确认的运营提醒；管理员可在 AI 调用分析页或顶部栏提醒下拉中确认提醒，也可通过默认关闭的 `AI_ALERT_WEBHOOK_ENABLED` / `AI_ALERT_WEBHOOK_URL` 配置在新提醒创建时发送站外 webhook。正式题目复审弹窗可生成缓存化 AI 复审建议，辅助管理员决定通过、修订或废弃；题目管理页支持按当前筛选检测题干精确重复或高相似题组，可处理用户提交的题干、答案、解析、知识点等纠错反馈，并可查看题目创建、编辑、删除和复审版本快照，辅助内容治理但不自动修改或回滚题库。详见 [docs/ROADMAP.md](docs/ROADMAP.md)。
