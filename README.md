...
当前工作重点是完成真实演示截图与 CI 实跑验证；登录、课程、刷题错题闭环、考试作答判分及投稿审核入库均已有浏览器 E2E 覆盖。AI 调用日志已保存真实输入/输出 Token，管理员可在 `ai.model-prices` 配置模型的 USD/百万 Token 单价后查看成本聚合，并可按用户设置每日调用配额（继承全局值或不限次数）；未配置价格不会产生虚假成本。管理端 AI 调用分析支持 7/14/30/90 天运营报告，展示与前一等长周期的环比，并对高失败率、延迟和调用量异常给出实时提醒。详见 [docs/ROADMAP.md](docs/ROADMAP.md)。
