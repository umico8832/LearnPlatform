...
- **数据统计**：学习趋势、正确率、知识点掌握雷达图、月度学习效果指标，以及 AI 资产真实查看与后续同题作答对照
...
Phase 20 演示验收与 AI 运营治理、Phase 21 前端信息架构与视觉体验优化均已完成，Phase 22 AI 学习效果验证已启动。平台会记录用户实际看到的 AI 学习资产，并在管理端对比阅读后同题作答与未阅读前/未阅读作答的正确率、样本量和反馈；统计明确为观察性关联，不把调用量当成学习提升。登录、课程、刷题错题、考试判分、投稿审核和高频页面巡检已有 5 条真实 Docker E2E 覆盖。后端默认测试 396 个、前端 Vitest 217 个均已通过；既有 Testcontainers 基线为 53 个，本轮因 Docker daemon 未运行未复验 V17。详见 [docs/ROADMAP.md](docs/ROADMAP.md)。
