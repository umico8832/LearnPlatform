...
public class AiCostCalculator {
    ...
    public static BigDecimal calculate(AiCallTicket.Price price,
                                       com.learnplatform.ai.model.ModelResult.Usage usage, boolean embedding) {
        if (price == null || usage == null || usage.inputTokens() == null || price.input() == null
                || price.input().signum() < 0 || !embedding && (usage.outputTokens() == null
                || price.output() == null || price.output().signum() < 0)) {
            return null;
        }
        BigDecimal input = BigDecimal.valueOf(usage.inputTokens()).multiply(price.input());
        BigDecimal output = embedding ? BigDecimal.ZERO
                : BigDecimal.valueOf(usage.outputTokens()).multiply(price.output());
