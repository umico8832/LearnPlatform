...
public class SubmissionDifficultyAssessmentService {
    ...
    public SubmissionDifficultyAssessmentService(AiInvocationService invocationService,
    ...
    public SubmissionDifficultyVO assessDifficulty(Long submissionId, Long userId) {
        ...
        try {
            String aiContent = invocationService.text("submission_difficulty_assessment", userId,
                    new AiService.AiPrompt(systemPrompt, userPrompt));
            return parseAiResponse(aiContent, submission.getDifficulty());
        } catch (Exception e) {
            if (e instanceof BusinessException business && business.getCode() == ResultCode.QUOTA_EXCEEDED.getCode()) {
                throw business;
            }
