...
final class AiEvaluationFixture {
    ...
    AiEvaluationFixture(AiEvaluationCorpus corpus, AiEvaluationCorpus.Case sample,
        ...
        this.sample = sample;
        provider = new AiEvaluationProvider(delegate, sample);
        ...
        AiCallGovernanceService governance = new AiCallGovernanceService(provider,
                new AiCostCalculator(config), config, logMapper, users);
        ...
        assetService = new QuestionLearningAssetService(provider, config, governance, assetMapper,
    ...
    List<String> check(boolean online) {
        ...
        if (!online) {
            check(failures, provider.getLastTokenUsage() == null, "fixture-usage-unknown");
        }
        return failures;
    }
