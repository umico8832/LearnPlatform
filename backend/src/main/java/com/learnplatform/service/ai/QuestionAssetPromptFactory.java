...
/**
 * 无状态的题目学习资产 Prompt 模板工厂，不读取业务数据也不调用 AI Provider。
 */
public final class QuestionAssetPromptFactory {
    ...
    private static Prompt buildFullExplanationPrompt(String questionContext) {
        ...
        return new Prompt(systemPrompt, "请解析这道题目：\n\n" + questionContext);
    }

    private static Prompt buildBeginnerExplanationPrompt(String questionContext) {
        String systemPrompt = "你是一位耐心的辅导老师，擅长用最简单的方式给零基础的学生讲解题目。\n\n"
                ...
                + "6. 最后做一个简短回顾，帮助记忆\n\n"
                + "使用 Markdown 格式输出。";
        return new Prompt(systemPrompt, "请用最简单的方式解析这道题目：\n\n" + questionContext);
    }

    private static Prompt buildStepByStepPrompt(String questionContext) {
        String systemPrompt = "你是一位注重方法论的辅导老师。\n\n"
                ...
                + "6. 最后给出「步骤速查表」方便回顾\n\n"
                + "使用 Markdown 格式输出。";
        return new Prompt(systemPrompt, "请拆解这道题目的解题步骤：\n\n" + questionContext);
    }

    private static Prompt buildWrongOptionAnalysisPrompt(String questionContext) {
        String systemPrompt = "你是一位精通出题心理学的教育专家。\n\n"
                ...
                + "6. 最后给出「防坑指南」速记要点\n\n"
                + "使用 Markdown 格式输出。";
        return new Prompt(systemPrompt, "请分析这道题目的错误选项：\n\n" + questionContext);
    }

    private static Prompt buildCommonMistakesPrompt(String questionContext) {
        String systemPrompt = "你是一位经验丰富的出题老师和阅卷专家。\n\n"
                ...
                + "使用 Markdown 格式输出。";
        return new Prompt(systemPrompt, "请分析这道题目的常见误区：\n\n" + questionContext);
    }

    private static Prompt buildVariantPrompt(String questionContext) {
        String systemPrompt = "你是一位专业的出题老师。\n\n"
                ...
                + "  \"difficulty\": 3\n"
                + "}";
        return new Prompt(systemPrompt, "基于以下题目生成变式题：\n\n" + questionContext);
    }

    private static Prompt buildVisualInteractivePrompt(String questionContext) {
        ...
        return new Prompt(systemPrompt, "请为以下题目生成可视化讲解数据（严格输出 JSON）：\n\n" + questionContext);
    }
