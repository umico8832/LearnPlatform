<template>
  ...
  >
    <div class="courseware-heading">
      <div><span class="courseware-kicker">互动课件</span><h3>ArrayDeque 的循环数组表示与访问</h3></div>
    ...
    <div class="array-shell" aria-live="polite">
      <div class="array-meta"><span>capacity = {{ visualization.capacity }}</span><span>j = {{ visualization.headIndex }}</span><span>n = {{ visualization.elements.length }}</span><span>get({{ visualization.accessIndex }})</span></div>
      ...
      <p v-if="currentState.showOrder" class="logical-order">逻辑 List 顺序：{{ visualization.elements.join(' → ') }}</p>
    </div>
...
interface DequeState { activeIndexes: number[]; description: string; showOrder: boolean }
const props = defineProps<{ visualization: ArrayDequeRepresentationCourseware }>()
const currentStep = ref(0)
const physicalAccessIndex = computed(() => (props.visualization.headIndex + props.visualization.accessIndex) % props.visualization.capacity)
...
const states = computed<DequeState[]>(() => [
  { activeIndexes: [props.visualization.headIndex], description: `j = ${props.visualization.headIndex}，n = ${props.visualization.elements.length}。逻辑下标 0 从 j 开始，不要求位于物理下标 0。`, showOrder: false },
  { activeIndexes: props.visualization.elements.map((_, logicalIndex) => (props.visualization.headIndex + logicalIndex) % props.visualization.capacity), description: 'ArrayDeque 复用循环数组布局；物理下标回绕不改变逻辑 List 的先后顺序。', showOrder: true },
  { activeIndexes: [physicalAccessIndex.value], description: `get(${props.visualization.accessIndex}) 访问逻辑元素 ${accessedElement.value}：(${props.visualization.headIndex}+${props.visualization.accessIndex}) mod ${props.visualization.capacity} = ${physicalAccessIndex.value}，直接读取 a[${physicalAccessIndex.value}]。`, showOrder: true },
])
