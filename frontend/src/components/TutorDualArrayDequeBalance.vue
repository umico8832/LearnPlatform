<template>
  <section
    ...
    tabindex="0"
    aria-label="DualArrayDeque 再平衡互动课件，使用左右方向键切换步骤"
  ...
  >
    ...
    <div class="stacks" aria-live="polite">
      <div class="stack">
        ...
        <div class="values">{{ displayedFront.join(' → ') || '∅' }}</div>
      </div>
      <div class="stack">
        <span>back（栈底 → 栈顶）</span>
        <div class="values">{{ displayedBack.join(' → ') || '∅' }}</div>
      </div>
    </div>
...
const states = computed(() => [
  {
    description: isBackHeavy.value
      ? 'back 的大小超过 front 的三倍，满足 balance 的重建条件。'
      : 'front 的大小超过 back 的三倍，满足 balance 的重建条件。',
  },
  {
